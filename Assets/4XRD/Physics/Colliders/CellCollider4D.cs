...
{
    public class CellCollider4D : global::Engine4D.Scripts.Colliders.Collider4D
    {
        ...
        public void MakeCell(Vector4 a1, Vector4 a2, Vector4 A1, Vector4 A2, Vector4 b1, Vector4 b2, Vector4 B1, Vector4 B2)
        {
            ...
            basis = new Matrix4x4(v1, v2, v3, v1.TripleCross(v2, v3));
        ...
        static Vector3 CheckFlat(Vector4 v)
        {
        ...
        public override Vector4 NP(Vector4 localPt)
        {
            ...
            if (Mathf.Approximately(distSq1, minDist)) { return ReturnNP(np1); }
            if (Mathf.Approximately(distSq2, minDist)) { return ReturnNP(np2); }
            if (Mathf.Approximately(distSq3, minDist)) { return ReturnNP(np3); }
            if (Mathf.Approximately(distSq4, minDist)) { return ReturnNP(np4); }
            if (Mathf.Approximately(distSq5, minDist)) { return ReturnNP(np5); }
            return ReturnNP(np6);
        }

        Vector4 ReturnNP(Vector3 v)
        {
            return basis * v + origin;
        }

        static Vector3 QuadNP(Vector3 p, Vector3 a, Vector3 b, Vector3 c, Vector3 d, ref bool inside)
        {
