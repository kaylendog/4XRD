...
namespace _4XRD.Physics.Colliders
{
    ...
    /// <summary>
    /// A collider for an arbitrary mesh.
    /// </summary>
    public class MeshCollider4D : Collider4D
    {
        /// <summary>
        /// A tetrahedral cell in the mesh.
        /// </summary>
        [Serializable]
        ...
        public static Vector4 NPTetrahedron(Vector4 localPt, Tetrahedron tetra, ref bool inside)
        {
            ...
            if (!inside)
            {
                ...
                if (Mathf.Approximately(distSq1, minDist)) { pt = np1; }
                else if (Mathf.Approximately(distSq2, minDist)) { pt = np2; }
                else if (Mathf.Approximately(distSq3, minDist)) { pt = np3; }
                else { pt = np4; }
            }
